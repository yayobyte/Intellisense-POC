  ...
  export const TextAreaAutocomplete = forwardRef<HTMLInputElement, TextAreaAutocompleteProps<any>>(
    (
      ...
      const renderAutocompleteList = () => {
        ...
        return (
            <ul
              className="bg-white border border-black/15 shadow-lg fixed text-left z-[20000] list-none mt-4 p-0 text-md h-32 overflow-y-auto focus:outline-none pointer-events-auto scroll-smooth"
              onMouseMove={(event: React.MouseEvent<HTMLUListElement>) => {
                event.preventDefault();
                event.stopPropagation();
              }}
              onWheel={(event: React.WheelEvent<HTMLUListElement>) => {
                event.preventDefault()
                event.stopPropagation();
              }}
              style={{
